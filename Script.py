...
def Manipulate_Baselile_Data(train):
    ...
    Assessments['stdGameTime'] = Assessments.groupby(['installation_id','game_session'])['game_time'].transform(np.std)
    Assessments['MaxGameTime'] = Assessments.groupby(['installation_id','game_session'])['game_time'].transform(np.max)
    Assessments['MeanGameTime'] = Assessments.groupby(['installation_id','game_session'])['game_time'].transform(np.mean)
    
    
    Assessments['stdGameEventCount'] = Assessments.groupby(['installation_id','game_session'])['event_count'].transform(np.std)
    Assessments['MaxGameEventCount'] = Assessments.groupby(['installation_id','game_session'])['event_count'].transform(np.max)
    Assessments['MeanGameEventCount'] = Assessments.groupby(['installation_id','game_session'])['event_count'].transform(np.mean)
