...
TopTitles = [item for item in Titles if item not in ['Bird Measurer (Assessment)']]
train.loc[train['event_code'].isin([4100]) & train.title.isin(TopTitles), 'Attempt'] = 1
...
train['TotalGameSessionTime'] = train.groupby(['installation_id', 'game_session'])['game_time'].transform(np.max)
train['TotalGameSessionEvents'] = train.groupby(['installation_id', 'game_session'])['event_count'].transform(np.max)
train = train.sort_values('timestamp', ascending=True)


train = train[['installation_id', 'game_session','game_time','type', 'event_id',
                           'event_code','timestamp', 'title','world', 'event_count',
                           'Attempt', 'IsAttemptSuccessful',  'TotalGameSessionTime',  'TotalGameSessionEvents']]

train.loc[train.type == ,'Assessment_timestamp'] = train[]

train = train.sort_values(['installation_id', 'game_session','timestamp'])


train['PastClips'] = \
    z = train.groupby(['installation_id', 'type'])[['game_session']].nunique()

train = train[train.Attempt == 1]
Model_Base = Assessments.drop_duplicates()
Model_Base['PastGames'] = Model_Base.groupby('installation_id')['game_session'].transform(
...
Model_Base = Model_Base[
    ['Attempts', 'Success', 'title', 'PartOfDay', 'world', 'GameSessionTime', 'GameEventsCount', 'PastGames']]
...
Model_Base = Model_Base.drop_duplicates()
